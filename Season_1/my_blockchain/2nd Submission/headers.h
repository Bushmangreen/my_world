...
*/
//Error macros
#define COMMAND_NOT_FOUND(x) fprintf(stderr, "nok: %s not found\n", x)
...
int createBlock(GlobalNode* nodes, int nid, char* bid);
...
int getFirst2Commands(Command* commands, int* index, char* buff, int beg, /*int end*/int* spaces, int spaceIndex);
...
bool bidExists(Node* node, char* bid);
//in bidExists
int mine_strcmp(char* s1, char* s2);

int reallocArr(GlobalNode* nodes);

int listNodes(GlobalNode* nodes, Command* flag);
//inside listNodes
...
// function creates new backup
void createBackup(GlobalNode *nodes, char* backupName);


// function that reads existing backup
